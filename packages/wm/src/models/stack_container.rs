...
impl StackContainer {
  ...
  pub fn selected_child(&self) -> Option<Container> {
    self.child_focus_order().next()
  }

  pub fn contains_selected_child(&self, window: &WindowContainer) -> bool {
    self
      .selected_child()
      .is_some_and(|child| child.id() == window.id())
  }
  ...
  pub fn to_dto(&self) -> anyhow::Result<ContainerDto> {
    ...
    Ok(ContainerDto::Stack(StackContainerDto {
      ...
      selected_child_id: self.selected_child().map(|child| child.id()),
